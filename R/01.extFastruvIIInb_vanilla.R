#' Removing unwanted variation from matched sequencing count data (without constraint)
#'
#' This function performs the extended fast version of ruvIIInb method (without constraint) to remove unwanted variation from matched sequencing count data that contains features from two modalities. Currently, only Negative Binomial model for UMI data is supported. It takes raw count matrix with features (e.g., genes/transcript and proteins/ADT in CITE-seq) as rows and samples (cells) as columns. 
...
function(Y1,Y2,M,
  ...
  # combine Y1 and Y2
  Y12 <- rbind(Y1,Y2) #for est. psi (not anymore as now we have sep. est. for psi for features from different domain)
  ...
  Mb[union(union(rownames(Y1)[ctl1], rownames(Y2)[ctl2]), rownames(Y12)[ctl3]),] <- 0 #still work when ctl3 != concat(ctl1, ctl2)
  # identical(rownames(Mb[union(union(rownames(Y1)[ctl1], rownames(Y2)[ctl2]), rownames(Y12)[ctl3]),] ),
  #           union(union(rownames(Y1)[ctl1], rownames(Y2)[ctl2]), rownames(Y12)[ctl3])) #T
  ...
  while(!conv) {
    ...
    # identical(names(wt.ctl1), rownames(Y1)[ctl1]) #T
    # identical(names(wt.ctl2), rownames(Y2)[ctl2]) #T
    # identical(names(wt.ctl3), rownames(Y12)[ctl3]) #T
    ...
    while(!conv.beta) {
      ...
      Z3.c     <- Z[extr_ctl3,] - gmean3[extr_ctl3] - rbind(Matrix::tcrossprod(alpha1[which(rownames(alpha1) %in% rownames(Y12)[ctl3]),], W1sub), #use the freshly updated one
                                                            Matrix::tcrossprod(alpha2[which(rownames(alpha2) %in% rownames(Y12)[ctl3]),], W2sub)) #use the freshly updated one
      #identical(rownames(rbind(Matrix::tcrossprod(alpha1[which(rownames(alpha1) %in% rownames(Y12)[ctl3]),], W1sub.old),
      #                         Matrix::tcrossprod(alpha2[which(rownames(alpha2) %in% rownames(Y12)[ctl3]),], W2sub.old))), names(gmean3[extr_ctl3]))
      # [1] TRUE
      ...
      # > identical(rownames(Matrix::tcrossprod(rbind(alpha3_1,alpha3_2),W3sub)), rownames(rbind(Matrix::tcrossprod(alpha1,W1sub), Matrix::tcrossprod(alpha2,W2sub))))
      # [1] TRUE
      bef <- Sys.time()
      ...
      # identical(union(union(rownames(Y1)[ctl1], rownames(Y2)[ctl2]), rownames(Y12)[ctl3]), rownames(Mb[union(union(rownames(Y1)[ctl1], rownames(Y2)[ctl2]), rownames(Y12)[ctl3]),])) #T
      aft <- Sys.time()
      ...
      lmu.hat <- gmean3 + Mb[,apply(Msub,1,which)] + rbind(Matrix::tcrossprod(alpha1,W1sub), Matrix::tcrossprod(alpha2,W2sub)) + Matrix::tcrossprod(rbind(alpha3_1,alpha3_2),W3sub) 
      #all true below
      # identical(rownames(Matrix::tcrossprod(rbind(alpha3_1,alpha3_2),W3sub)), names(gmean3))
      # identical(rownames(Matrix::tcrossprod(rbind(alpha3_1,alpha3_2),W3sub)), rownames(Mb[,apply(Msub,1,which)]))
      # identical(rownames(Matrix::tcrossprod(rbind(alpha3_1,alpha3_2),W3sub)), rownames(rbind(Matrix::tcrossprod(alpha1,W1sub), Matrix::tcrossprod(alpha2,W2sub))))
      ...
      loglik.tmp <- temp #length = "sub" sample size
      
      # check degenerate case
      if(iter>=1) {
        degener<- sum(loglik)>sum(loglik.tmp) #before inner loop vs. updated by latest inner iter
        ...
        if(!degener) {
          ...
          best.wtctl1 <- wt.ctl1 ; best.wtctl2 <- wt.ctl2 ; best.wtctl3 <- wt.ctl3
          #
          iter <- iter + 1 #got update as above!
          halving <- 0
        }
      }
    ...
    # identical(rownames(best.Mb[,apply(Msub,1,which)] ), rownames(W3a3)) #T
    # identical(rownames(best.Mb[,apply(Msub,1,which)] ), rownames(rbind(W1a1, W2a2))) #T
    offs.psi <- offs[,psi.idx]
    ...
    if(conv) {
      ...
      W1[subsamples.org[subsubsamples.org],] <- W1sub[1:length(subsubsamples.org),] #insert estimates from Wsub
      ...
      while(!conv.W1 | !conv.W2 | !conv.W3) { #for any one of them not yet converge
        if(!conv.W1){
          ...
          if(nW1>1 & ortho.W)  
            W1 <- matlib::GramSchmidt(W1)
          ##
          crit.W1 <- mean( (abs(W1-W1.old)/abs(W1.old))^2) #easy to go inf...
  ...
  # identical(union(union(rownames(Y1)[ctl1], rownames(Y2)[ctl2]), rownames(Y12)[ctl3]), rownames(Mb.all[union(union(rownames(Y1)[ctl1], rownames(Y2)[ctl2]), rownames(Y12)[ctl3]),]))
  # T
