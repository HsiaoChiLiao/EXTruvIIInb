#' Removing unwanted variation from matched sequencing count data (with CCA constraint)
#'
#' This function performs the extended fast version of ruvIIInb method (with CCA constraint) to remove unwanted variation from matched sequencing count data that contains features from two modalities. Currently, only Negative Binomial model for UMI data is supported. It takes raw count matrix with features (e.g., genes/transcript and proteins/ADT in CITE-seq) as rows and samples (cells) as columns. 
...
#' @param tau_1 the weight for the constraint controlling the correlation between W1 and W3. A bigger value leads to lower correlation. Default is 1.
#' @param tau_2 the weight for the constraint controlling the correlation between W2 and W3. A bigger value leads to lower correlation. Default is 1. 
...
function(Y1,Y2,M,
  ...
  while(!conv) {
    ...
    while(!conv.beta) {
      ...
      #W3 (using nc3)
      # k <- k3 #because before we need k1=k2=k3
      ...
      ##update all cells, all k at once
      {
        ...
        ## CCA parts
        AB.list <- tryCatch({
          A <- 2*tau_1*(diag(nrow(W3sub.old)) - W3sub.old %*% Matrix::chol2inv(chol(cp.w3sub.old)) %*% t(W3sub.old)) %*% (W1sub %*% Matrix::chol2inv(chol(cp.w1sub)) %*% t(W1sub) %*% W3sub.old %*% Matrix::chol2inv(chol(cp.w3sub.old)))
          B <- 2*tau_2*(diag(nrow(W3sub.old)) - W3sub.old %*% Matrix::chol2inv(chol(cp.w3sub.old)) %*% t(W3sub.old)) %*% (W2sub %*% Matrix::chol2inv(chol(cp.w2sub)) %*% t(W2sub) %*% W3sub.old %*% Matrix::chol2inv(chol(cp.w3sub.old)))
          AB.list <- list(A, B)
        }, error = function(err){
          A <- 2*tau_1*(diag(nrow(W3sub.old)) - W3sub.old %*% solve(cp.w3sub.old) %*% t(W3sub.old)) %*% (W1sub %*% solve(cp.w1sub) %*% t(W1sub) %*% W3sub.old %*% solve(cp.w3sub.old))
          B <- 2*tau_2*(diag(nrow(W3sub.old)) - W3sub.old %*% solve(cp.w3sub.old) %*% t(W3sub.old)) %*% (W2sub %*% solve(cp.w2sub) %*% t(W2sub) %*% W3sub.old %*% solve(cp.w3sub.old))
          AB.list <- list(A, B)
          return(AB.list)
        })
      ...
      W3sub  <- (z.w.a - A - B) %*% Matrix::chol2inv(chol(lambda.a3 + a.w.a))
      # W3sub  <- (z.w.a - A - B) %*% solve(lambda.a3 + a.w.a)
      
      if((k1 == 1) & (k2 == 1)){
        trace_sub.w13[iter] <- sum(diag(1/sqrt(cp.w1sub) %*% crossprod(W1sub,W3sub) %*% Matrix::chol2inv(chol(t(W3sub)%*% W3sub)) %*% crossprod(W3sub, W1sub) %*% 1/sqrt(cp.w1sub)))
        trace_sub.w23[iter] <- sum(diag(1/sqrt(cp.w2sub) %*% crossprod(W2sub,W3sub) %*% Matrix::chol2inv(chol(t(W3sub)%*% W3sub)) %*% crossprod(W3sub, W2sub) %*% 1/sqrt(cp.w2sub)))
      }
      
      # doesn't work when k1=k2=k3=1
      if((k1 > 1) & (k2 > 1)){
        trace_sub.w13[iter] <- sum(diag(matrixsqrtinv(cp.w1sub) %*% crossprod(W1sub,W3sub) %*% Matrix::chol2inv(chol(t(W3sub)%*% W3sub)) %*% crossprod(W3sub, W1sub) %*% matrixsqrtinv(cp.w1sub)))
        trace_sub.w23[iter] <- sum(diag(matrixsqrtinv(cp.w2sub) %*% crossprod(W2sub,W3sub) %*% Matrix::chol2inv(chol(t(W3sub)%*% W3sub)) %*% crossprod(W3sub, W2sub) %*% matrixsqrtinv(cp.w2sub)))
      }
    ...
    gmean3 <- best.gmean3
    # # new changes
    # best.psi <- psi (not here! let degener decide)
    ...
    if(conv){
      ...
      for(block in 1:nb) {
        ...
        ##update all cells, all k at once
        {
          ...
          cp.w1 <- crossprod(W1[start.idx:end.idx,,drop=FALSE], W1[start.idx:end.idx,,drop=FALSE]) #crossprod using W1 freshly updated above
          cp.w2 <- crossprod(W2[start.idx:end.idx,,drop=FALSE], W2[start.idx:end.idx,,drop=FALSE]) #crossprod using W2 freshly updated above
          cp.w3 <- crossprod(W3[start.idx:end.idx,,drop=FALSE], W3[start.idx:end.idx,,drop=FALSE]) #there's no w3.old
          
          ## CCA parts
          AB.list <- tryCatch({
            A <- 2*tau_1*(diag(nrow(W3[start.idx:end.idx,,drop=FALSE])) - W3[start.idx:end.idx,,drop=FALSE] %*% Matrix::chol2inv(chol(cp.w3)) %*% t(W3[start.idx:end.idx,,drop=FALSE])) %*% (W1[start.idx:end.idx,,drop=FALSE] %*% Matrix::chol2inv(chol(cp.w1)) %*% t(W1[start.idx:end.idx,,drop=FALSE]) %*% W3[start.idx:end.idx,,drop=FALSE] %*% Matrix::chol2inv(chol(cp.w3)))
            B <- 2*tau_2*(diag(nrow(W3[start.idx:end.idx,,drop=FALSE])) - W3[start.idx:end.idx,,drop=FALSE] %*% Matrix::chol2inv(chol(cp.w3)) %*% t(W3[start.idx:end.idx,,drop=FALSE])) %*% (W2[start.idx:end.idx,,drop=FALSE] %*% Matrix::chol2inv(chol(cp.w2)) %*% t(W2[start.idx:end.idx,,drop=FALSE]) %*% W3[start.idx:end.idx,,drop=FALSE] %*% Matrix::chol2inv(chol(cp.w3)))
            AB.list <- list(A, B)
          }, error = function(err){
            A <- 2*tau_1*(diag(nrow(W3[start.idx:end.idx,,drop=FALSE])) - W3[start.idx:end.idx,,drop=FALSE] %*% solve(cp.w3) %*% t(W3[start.idx:end.idx,,drop=FALSE])) %*% (W1[start.idx:end.idx,,drop=FALSE] %*% solve(cp.w1) %*% t(W1[start.idx:end.idx,,drop=FALSE]) %*% W3[start.idx:end.idx,,drop=FALSE] %*% solve(cp.w3))
            B <- 2*tau_2*(diag(nrow(W3[start.idx:end.idx,,drop=FALSE])) - W3[start.idx:end.idx,,drop=FALSE] %*% solve(cp.w3) %*% t(W3[start.idx:end.idx,,drop=FALSE])) %*% (W2[start.idx:end.idx,,drop=FALSE] %*% solve(cp.w2) %*% t(W2[start.idx:end.idx,,drop=FALSE]) %*% W3[start.idx:end.idx,,drop=FALSE] %*% solve(cp.w3))
            AB.list <- list(A, B)
            return(AB.list)
          })
        ...
        W3[start.idx:end.idx,]  <- (z.w.a - A - B) %*% Matrix::chol2inv(chol(lambda.a3 + a.w.a)) 
      }
        ...
        if(!conv.W3){
          ...
          for(block in 1:nb) {
            ...
            ##update all cells, all k at once
            {
              ...
              cp.w1 <- crossprod(W1[start.idx:end.idx,,drop=FALSE], W1[start.idx:end.idx,,drop=FALSE]) #crossprod using W1 freshly updated above
              cp.w2 <- crossprod(W2[start.idx:end.idx,,drop=FALSE], W2[start.idx:end.idx,,drop=FALSE]) #crossprod using W2 freshly updated above
              cp.w3 <- crossprod(W3[start.idx:end.idx,,drop=FALSE], W3[start.idx:end.idx,,drop=FALSE]) #need to use the old one
            ...
            # print("updating big W3")
            # print(paste0("inv tau1 part: ", as.numeric(solve(lambda.a3 + a.w.a))/as.numeric(A), "; inv tau2 part: ", as.numeric(solve(lambda.a3 + a.w.a))/as.numeric(B)))
            # print(paste0("zwa tau1 part: ", as.numeric(z.w.a/A), "; zwa tau2 part: ", as.numeric(z.w.a/B)))
          }
          ...
          if((k1 == 1) & (k2 == 1)){
            trace_w13[iter.W3] <- sum(diag(1/sqrt(all.cp.w1) %*% crossprod(W1,W3) %*% Matrix::chol2inv(chol(t(W3)%*% W3)) %*% crossprod(W3, W1) %*% 1/sqrt(all.cp.w1)))
            trace_w23[iter.W3] <- sum(diag(1/sqrt(all.cp.w2) %*% crossprod(W2,W3) %*% Matrix::chol2inv(chol(t(W3)%*% W3)) %*% crossprod(W3, W2) %*% 1/sqrt(all.cp.w2)))
          }
          
          # doesn't work when k1=k2=k3=1
          if((k1 > 1) & (k2 > 1)){
            trace_w13[iter.W3] <- sum(diag(matrixsqrtinv(all.cp.w1) %*% crossprod(W1,W3) %*% Matrix::chol2inv(chol(t(W3)%*% W3)) %*% crossprod(W3, W1) %*% matrixsqrtinv(all.cp.w1)))
            trace_w23[iter.W3] <- sum(diag(matrixsqrtinv(all.cp.w2) %*% crossprod(W2,W3) %*% Matrix::chol2inv(chol(t(W3)%*% W3)) %*% crossprod(W3, W2) %*% matrixsqrtinv(all.cp.w2)))
          }
